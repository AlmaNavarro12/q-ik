...
class PDF extends FPDF {
    ...
    function Footer() {
        if($this->idorden >= 1558){
            ...
            $this->SetY(-29);
            $this->RoundedRect(140, ($this->GetY() - 40), 60, 48, 3, 'D');
            if ($this->firma != "") {
                $this->Image($this->firma, 140, ($this->GetY() - 42), 60, 0, 'png');
            }
            $this->SetX(145);
            $this->SetTextColor(0, 0, 0);
            $this->SetFont('Arial', 'B', 8);
            ...
            $this->SetFont('Arial', 'B', 8);
            $this->Cell(65, 10, '', 0, 0, 'L');
            $this->Cell(65, 10, iconv('UTF-8', 'windows-1252', 'Pagina ' . $this->PageNo() . ' de {nb}'), 0, 0, 'C');
            $this->Cell(65, 10, '', 0, 0, 'R');
        }
...
$reporte = $ci->getOrdenAux($dinicio, $dfin, $cliente, $servicio);
...
$pdf->idorden = $noorden ?? '00';
$pdf->title = $titulo;
$pdf->fechaservicio = $fechaservicio2 ?? 'No definida';
$pdf->firma = $firma ?? '';
$pdf->encargado = $encargado ?? 'No definido';

foreach ($reporte as $reporteactual) {
    $idservicio = $reporteactual['idorden'];
    $status_servicio = $reporteactual['estado_orden'];
    ...
    if($tipounidad == 1){
        ...
        if($otrosmolduras == ''){
            $otrosmolduras = "Otros (especifique)";
        }

        if($otrostablero == ""){
            $otrostablero = "Otros (especifique)";
        }

        if($otroscableado == ""){
            $otroscableado = "Otros (especifique)";
        }

        if($otroscorriente == ""){
            $otroscorriente = "Otros (especifique)";
        }

        $pdf->title = "Recepción del vehiculo";
        $pdf->AliasNbPages();
        $pdf->AddPage();
        ...
        foreach ($divD as $damactual) {
            ...
            if($damactual != 9){
                ...
                $pdf->SetTextColor($rgbtxt[0], $rgbtxt[1], $rgbtxt[2]);
                $pdf->Row(Array(chr(149) . " " .iconv('UTF-8', 'windows-1252',$danho), chr(149) . " " .iconv('UTF-8', 'windows-1252',"No aplica imagen")));
            }
        }
        $pdf->Ln(5);
        $pdf->SetTextColor($rgbbld[0], $rgbbld[1], $rgbbld[2]);
        ...
        $pdf->SetTextColor($rgbtxt[0], $rgbtxt[1], $rgbtxt[2]);
                $pdf->Row(Array(chr(149) . " " .iconv('UTF-8', 'windows-1252',$moldura), chr(149) . " " .iconv('UTF-8', 'windows-1252',"No aplica imagen")));
            }
        }
        ...
        foreach ($divT as $tabactual) {
            ...
            $image_type = explode(';', $image_extension)[0];
                ...
                $pdf->SetTextColor($rgbtxt[0], $rgbtxt[1], $rgbtxt[2]);
                $pdf->Row(Array(chr(149) . " " .iconv('UTF-8', 'windows-1252',$tablero), chr(149) . " " .iconv('UTF-8', 'windows-1252',"No aplica imagen")));
            }
        ...
        foreach ($divcorriente as $corrienteactual) {
            ...
            $pdf->SetTextColor($rgbtxt[0], $rgbtxt[1], $rgbtxt[2]);
            $pdf->Row(Array(chr(149) . " " .iconv('UTF-8', 'windows-1252',$ccorriente)));
        }
        ...
        $pdf->idorden = $noorden;
        $pdf->fechaservicio = $fechaservicio2;
        $pdf->title = "Certificado de Servicio";
        $pdf->firma = $firma;
        $pdf->encargado = $encargado;
